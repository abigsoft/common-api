...
class Category
{
     ...
     * @param string $full 格式化后分类名称
     */
    public function __construct($field = array())
    {
        $this->field['id'] = $field[0] ?? 'idx';
        $this->field['pid'] = $field[1] ?? 'parent';
        $this->field['title'] = $field[2] ?? 'name';
        $this->field['full'] = $field[3] ?? 'full';
    }
    ...
    private function _searchList($id = 0, $space = "")
    {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            $childs[$i][$this->field['full']] = ($space ? $space . $pre : "") . $childs[$i][$this->field['title']];
